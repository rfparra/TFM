...
def spiral(outer_diam: float, inner_diam: float, wire_diam: float, segment_count: int):
    ...
    N=5.5 #Numero de vueltas
    h=2.2 #Altura de la bobina en mm
...
def spiral9(outer_diam3: float, inner_diam: float, wire_diam2: float, segment_count: int):
    ...
    N=7.5 #Numero de vueltas
    h=0.9 #Altura de la bobina en mm
...
def figure_of_8_wire_path(
    ...
    spiral_1 = (
        path + np.array((0, 7.842, -winding_casing_distance))[:, None]
    )
    ...
    spiral_3 = np.fliplr(
         #* np.array((-1, 1, 1))[:, None]
        path + np.array((8.223, 0, -winding_casing_distance))[:, None]
    )
    ...
    spiral_9 = (
        path + np.array((-1.504, -1.451, -winding_casing_distance))[:, None]
    )
...
tms_coil.write("EstimuladorM05.tcd")