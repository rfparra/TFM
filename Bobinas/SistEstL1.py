...
def figure_of_8_wire_path(
    ...
    spiral_2 = np.fliplr(
        path + np.array((0, 11.497, -winding_casing_distance))[:, None] # Global position of the first core
    )

    path = spiral3(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_3 = np.fliplr(
         #* np.array((-1, 1, 1))[:, None]
        path + np.array((12.386, 0, -winding_casing_distance))[:, None] # Global position of the second coil
    )

    path = spiral4(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_4 = np.fliplr(
         #* np.array((-1, 1, 1))[:, None]
        path + np.array((12.386, 0, -winding_casing_distance))[:, None] # Global position of the second core
    )

    path = spiral5(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_5 = (
        path + np.array((0, -11.497, -winding_casing_distance))[:, None] # Global position of the third coil
    )

    path = spiral6(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_6 = np.fliplr(
         #* np.array((-1, 1, 1))[:, None]
        path + np.array((0, -11.497, -winding_casing_distance))[:, None] # Global position of the third core
    )

    path = spiral7(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_7 = np.fliplr(
         #* np.array((-1, 1, 1))[:, None]
        path + np.array((-12.386, 0, -winding_casing_distance))[:, None] # Global position of the fourth coil
    )

    path = spiral8(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_8 = np.fliplr(
         #* np.array((-1, 1, 1))[:, None]
        path + np.array((-12.386, 0, -winding_casing_distance))[:, None] # Global position of the fourth core
    )
