...
def spiral2(outer_diam2: float, inner_diam: float, wire_diam: float, segment_count: int):
    ...
    # Calculates the radius of every line segment in the spiral
    radius_loop = outer_diam2 / 2 - wire_diam / 2 * phi / (2 * np.pi)
    # Coil pitch
...
def spiral3(outer_diam3: float, inner_diam: float, wire_diam: float, segment_count: int):
    """Creates a spiral path based on the inner and outer diameter of the coil as well as
    the diameter of the wire and the segment count.
    ...
    # Calculates the radius of every line segment in the spiral
    radius_loop = outer_diam3 / 2 - wire_diam / 2 * phi / (2 * np.pi)
    # Coil pitch
    pitch=-h/(2*np.pi*N) 
    # Calculates the cartesian coordinates of the spiral
    path = np.array(
...
def spiral4(outer_diam4: float, inner_diam: float, wire_diam: float, segment_count: int):
    """Creates a spiral path based on the inner and outer diameter of the coil as well as
    the diameter of the wire and the segment count.
    ...
    # Calculates the cartesian coordinates of the spiral
    path = np.array(
        [outer_diam4 * np.cos(phi), outer_diam4 * np.sin(phi), pitch*phi]
    )
...
def figure_of_8_wire_path(
    ...
    spiral_1 = (
        path + np.array((-element_distance / 2, 0, -winding_casing_distance))[:, None] # Global position of the first coil
    )

    path = spiral2(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_2 = np.fliplr(
        path + np.array((-element_distance / 2, 0, -winding_casing_distance))[:, None] # Global position of the second coil
    )

    path = spiral3(outer_diam3, inner_diam, wire_diam, segment_count)
    spiral_3 = np.fliplr(
        path + np.array((-element_distance / 2, 0, -winding_casing_distance))[:, None] # Global position of the third coil
    )
...
tms_coil.write("BobinaNUEVA.tcd")