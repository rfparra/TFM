...
This is a 4 coil stimulator compound by coils L5
"""
...
def spiral16(outer_diam2: float, inner_diam: float, wire_diam: float, segment_count: int):
    """Creates a spiral path based on the inner and outer diameter of the coil as well as
    the diameter of the wire and the segment count.
...
def figure_of_8_wire_path(
    ...
    spiral_1 = (
        path + np.array((0, 1.451, -winding_casing_distance))[:, None] # Global position of the first coil
    )

    path = spiral2(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_2 = np.fliplr(
        path + np.array((0, 1.451, -winding_casing_distance))[:, None] # Global position of the first core
    )

    path = spiral3(outer_diam, inner_diam, wire_diam, segment_count2)
    spiral_3 = np.fliplr(
        path + np.array((1.504, 0, -winding_casing_distance))[:, None] # Global position of the second coil
    )

    path = spiral4(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_4 = np.fliplr(
        path + np.array((1.504, 0, -winding_casing_distance))[:, None] # Global position of the second core
    )
    path = spiral5(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_5 = (
        path + np.array((0, -1.451, -winding_casing_distance))[:, None] # Global position of the third coil
    )

    path = spiral6(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_6 = np.fliplr(
        path + np.array((0, -1.451, -winding_casing_distance))[:, None] # Global position of the third core
    )

    path = spiral7(outer_diam, inner_diam, wire_diam, segment_count)
    spiral_7 = np.fliplr(
        path + np.array((-1.504, 0, -winding_casing_distance))[:, None] # Global position of the fourth coil
    )

    path = spiral8(outer_diam2, inner_diam, wire_diam, segment_count)
    spiral_8 = np.fliplr(
        path + np.array((-1.504, 0, -winding_casing_distance))[:, None] # Global position of the fourth core
    )
...
outer_diam = 4 #Esto ya nos da igual, solo se va a tener un diametro
inner_diam = 0.02
outer_diam2 = 40
outer_diam3= 1.2
outer_diam4= 7
