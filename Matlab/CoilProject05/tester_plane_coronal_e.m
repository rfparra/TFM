...
tic
prec         = 1e-4;            %    Precision 
Field 	     = bemf3_inc_field_electric_core(strcoil, pointsXZ, mu0, Moments, GEOM, prec);
fieldPlaneTime = toc  

%%  Assign object types to observation points and eliminate the inaccurate E-field within the core
obsPointCore = assign_tissue_type_volume(pointsXZ, GEOM.normals, GEOM.Center, Indicator(1:length(GEOM.t)));
in              = obsPointCore > 0;    
out             = obsPointCore == 0; 
Field(in, :)   = 0; 
...
axis equal; axis([xmin xmax zmin zmax]);
