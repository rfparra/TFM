...
dIdt = 2*pi*10*I0;           %   Amperes/sec (2*pi*I0/period), for electric field for 10 kHz


%%  First helix with a varying pitch - Hanbing
turns       = 23.75;                               %   number of turns (shold be 19?)
theta0      = turns*2*pi;                       %   how many turns (one turn is 2*pi)
twist       = 0;                            %   twist per delta theta
N           = 1000;                              %   subdivisions total
theta       = linspace(0, theta0, N);           %   running angle in radians
delta_theta = theta(2) - theta(1);              %   angle step in radians


d       = 0.0008;                                %   conductor diameter in m (0.004 mm) 
radius1 = 0.00845;                                %   helix radius, m checks
pitch1  = 0.0008;                               %   pitch amplitude
...
[strcoil1, check]        = meshwire(Pcenter, d, d, M, flag, sk, twist);  %    Wire model    
[P1, t1]                 = meshsurface(Pcenter, d, d, M, flag, twist);   %    CAD mesh (optional, slow) 


mu0                         = 1.25663706e-006;  %   Magnetic permeability of vacuum(~air)
prec                        = 1e-4;
Inductance1                 = bemf6_inductance_neumann_integral(strcoil1, mu0, prec)


% %   Rotate the first helix by 0 deg about the z-axis
% angle           = 0;
% P1              = meshrotate2(P1, [0 0 1], angle);
% strcoil1.Pwire  = meshrotate2(strcoil1.Pwire, [0 0 1], angle);
...
d       = 0.0008;                                %   conductor diameter in m (0.004 mm) 
radius2 = 0.00925;                                %   helix radius, m checks
pitch2  = 0.0008;                               %   pitch amplitude
...
mu0                         = 1.25663706e-006;  %   Magnetic permeability of vacuum(~air)
prec                        = 1e-4;
Inductance2                 = bemf6_inductance_neumann_integral(strcoil2, mu0, prec)
...
turns       = 11.875;                               %   number of turns (shold be 19?)
theta0      = turns*2*pi;                       %   how many turns (one turn is 2*pi)
twist       = 0;                            %   twist per delta theta
N           = 1000;                              %   subdivisions total
theta       = linspace(0, theta0, N);           %   running angle in radians
...
d       = 0.0008;                                %   conductor diameter in m (0.004 mm) 
radius3 = 0.01005;                                %   helix radius, m checks
pitch3  = 0.0016;                               %   pitch amplitude
x       = radius3*cos(theta);                   %   segment
y       = radius3*sin(theta);                   %   segment
z       = zeros(1, N);
delta   = pitch3*delta_theta/pi; 
for m = 2:N
    z(m)  = z(m-1) + 0.5*(1 - sign(sin(theta(m))))*delta;
end
%       Do smooting of the path
