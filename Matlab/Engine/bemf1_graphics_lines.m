...
%   Plots coil centerline and handle
    ...
    starthandle = Target - 0e-3*dirline;
    ...
    if strcmp(ind, 'xyz')  
        ...
        plot3(pointshandle(:, 1), pointshandle(:, 2), pointshandle(:, 3), '-r', 'lineWidth', 3);
...
%         p = patch('vertices', scale*S.P+repmat(Target, n, 1), 'faces', S.t); p.FaceColor = 'c'; p.EdgeColor = 'none'; p.FaceAlpha = 1.0;
    ...
    if strcmp(ind, 'xy')
        ...
        plot(scale*pointshandle(:, 1), scale*pointshandle(:, 2), '-r', 'lineWidth', 3);
    end
    if strcmp(ind, 'xz')
        hold on;
        plot(scale*pointsline(:, 1), scale*pointsline(:, 3), '-k', 'lineWidth', 3);
        plot(scale*pointshandle(:, 1), scale*pointshandle(:, 3), '-r', 'lineWidth', 3);
    end
    if strcmp(ind, 'yz')
        hold on;
        plot(scale*pointsline(:, 2), scale*pointsline(:, 3), '-k', 'lineWidth', 3);
        plot(scale*pointshandle(:, 2), scale*pointshandle(:, 3), '-r', 'lineWidth', 3);
    end
end