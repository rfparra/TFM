function [name, tissue, cond, enclosingTissueIdx] = tissue_index_read(index_name)
...
%   Copyright WAW/SNM 2019-2020
    ...
    while ~feof(index_file)
        ...
        if(isempty(tempFileName))
            error(['No file name found on line ' num2str(lineCounter) ' of tissue_index.txt']);
        end
        %Check that the file exists
        if ~exist(tempFileName, 'file')
            error(['File [' tempFileName '] referenced on line ' num2str(lineCounter) ' of tissue_index.txt does not exist.']);
        end

        %Check that the tissue has a name associated with it
        if(isempty(tempTissueName))
            error(['No tissue name found on line ' num2str(lineCounter) ' of tissue_index.txt']);
        end

        %Check that the tissue has a conductivity associated with it
        if(isempty(tempConductivity))
            error(['No tissue conductivity found on line ' num2str(lineCounter) ' of tissue_index.txt']);
        end
        ...
        if(isnan(tempTissueConductivity))
            error(['Tissue conductivity on line ' num2str(lineCounter) ' of tissue_index.txt is not numeric']);
        end

        %Check that the tissue has an enclosing tissue associated with it
        if(isempty(tempEnclosingTissueName))
            error(['Enclosing tissue is not specified on line ' num2str(lineCounter) ' of tissue_index.txt']);
        end
