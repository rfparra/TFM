...
obsPointTissues = assign_tissue_type_volume(pointsXZ, normals, Center, Indicator);
...
Epri           = zeros(Ms*Ms, 3);
Esec           = zeros(Ms*Ms, 3);
Epri(in, :)    = bemf3_inc_field_electric_core(strcoil, pointsXZ(in, :), mu0, prec);        %  Incident coil field
Esec(in, :)    = bemf5_volume_field_electric(pointsXZ(in, :), c, P, t, Center, Area, normals, R, prec);
Etotal         = Epri + Esec;   
fieldPlaneTime = toc  
